...
const JobsPage = async ({
  ...
  const searchLoction = location as string | undefined;
  ...
  const jobs = await prisma.job.findMany({
    where: {
      AND: [
        ...
        location
          ? { location: { contains: searchLoction, mode: 'insensitive' } }
          : {},
      ],
    },
    ...
    <div className='space-y-8'>
      <div className='bg-white p-6 rounded-lg shadow-sm'>
        ...
        <form className='grid gap-4 md:grid-cols-3'>
          ...
          <button
            type='submit'
            className='md:max-w-32  bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700'
          >
            Search
          </button>
        </form>
